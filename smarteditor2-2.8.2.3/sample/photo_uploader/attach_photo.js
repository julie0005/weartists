	...
	var rFilter = /^(image\/bmp|image\/gif|image\/jpg|image\/jpeg|image\/png|image\/ai)$/i;  
	var rFilter2 = /^(bmp|gif|jpg|jpeg|png|ai)$/i; 
	var nTotalSize = 0;
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			for (var i = 0, j = nImageFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					alert("이미지파일 (jpg,gif,png,bmp,ai,jpeg)만 업로드 가능합니다.");
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						alert("이미지 파일(jpg,gif,png,bmp,jpeg,ai)만 업로드 하실 수 있습니다. ("+sFileName+")");
					}else{
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 	    	...
 	    	sFiletype : "*.jpg;*.png;*.bmp;*.gif;*.ai;*.jpeg",						//허용할 파일의 형식. ex) "*", "*.*", "*.jpg", 구분자(;)	
 	    	sMsgNotAllowedExt : 'JPG, GIF, PNG, BMP, JPEG, AI 확장자만 가능합니다',	//허용할 파일의 형식이 아닌경우에 띄워주는 경고창의 문구
